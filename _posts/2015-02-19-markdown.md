...
这几天试着用github博客系统发布用[markdown](http://daringfireball.net/projects/markdown)写的日志，发现GitHub默认的markdown解析器[kramdown](http://kramdown.gettalong.org/index.html)对一些程序员比较实用的markdown扩展支持不好，比如：
...
这些特性已经被一些Windows端的markdown编辑器支持, 比如让人眼前一亮的[飞象马克](http://www.maxiang.info/)。
所以花了一点时间把飞象马克宣传页的markdown特性调到GitHub博客上。

最后的结果没有想象的好, 主要是GitHub的Jekyll引擎对流程图和时序图的语法不支持，必须写额外的转换[插件](http://jekyllrb.com/docs/plugins/)，但就算写好了插件提交到GitHub上，GitHub因为自身的安全机制也不会替你自动加载，所以这个问题暂时无解。(有谁知道请告诉我)

我最后用的workaround是直接在markdown文档中插入原始html代码，跳过Jekyll的转换。虽然比较ugly，但是除了语法本身，额外的代码不多，先忍着吧。
